...
class StretchyController: UIViewController,UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let req5day = Alamofire.request(url5day, method: .post, encoding: URLEncoding.methodDependent, headers: [:]).responseString { (response) in
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        var forecastDt = dateFormatter1.string(from: NSDate(timeIntervalSince1970: Double(forecastinfo[0].list[indexPath.row].dt)) as Date)
        if (Int(forecastDt) ?? nil)! > 12{
            var forecastDt1 = dateFormatter1.string(from: NSDate(timeIntervalSince1970: Double(forecastinfo[0].list[indexPath.row].dt)-3600*12) as Date)
